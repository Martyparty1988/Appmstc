...
export function NewConversationModal({
  ...
  const conversationType = useMemo(() => {
    ...
    if (selectedUsers.length > 1) return 'group';
    return 'direct';
  }, [selectedUsers.length, selectedProjectId]);
  ...
  const canCreate = useMemo(() => {
    ...
    if (selectedUsers.length > 1 && !groupName.trim()) return false;
    return true;
  }, [activeTab, selectedUsers.length, groupName, selectedProjectId]);
  ...
  const handleCreate = async () => {
    ...
    if (activeTab === 'project' && selectedProjectId) {
      ...
      conversationId = await createConversation(`Projekt: ${selectedProjectId}`);
    } else {
      // Direct nebo group
      const name = selectedUsers.length > 1 ? groupName.trim() : undefined;
      conversationId = await createConversation(name);
    }
    ...
    return (
      ...
      >
        ...
        <div className="relative">
          ...
          />
          {user.isOnline && (
            <div className="absolute bottom-0 right-0 w-3 h-3 bg-success-500 rounded-full border-2 border-white dark:border-slate-900" />
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <ModalContent className="p-0">
        ...
        {/* Group name input */}
        {activeTab === 'users' && selectedUsers.length > 1 && (
          <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
            <Input
              label="Název skupiny"
            ...
            <>
              {users.status === 'loading' ? (
                ...
                <EmptyState
                  ...
                  message={searchQuery ? 'Zkuste jiný hledaný výraz' : 'Nejsou k dispozici žádní uživatelé'}
                  className="py-8"
                />
              ) : (
            ...
            <>
              {projects.status === 'loading' ? (
                ...
                <EmptyState
                  ...
                  message={searchQuery ? 'Zkuste jiný hledaný výraz' : 'Nejsou k dispozici žádné projekty'}
                  className="py-8"
                />
              ) : (
                <div className="space-y-2">
            ...
            {activeTab === 'users' && selectedUsers.length > 0 && (
              <span>
                {selectedUsers.length === 1
                  ? 'Přímá zpráva'
                  : `Skupina (${selectedUsers.length} členů)`
                }
              </span>
            )}
